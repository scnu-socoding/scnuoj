...
<?php if ($model->canViewResult()) : ?>
    <div class="alert alert-light animate__animated animate__fadeIn animate__faster">
        ...
        <?php if ($model->getTestCount()) : ?>
            ...
            <?php else : ?>
                你没有权限查看提交详情，在管理员重新给予你权限前，本页只可用于查看编译出错信息。
            <?php endif; ?>
        <?php else : ?>
            暂时无法获取本题测试点详情。
            <?php $flag = 1; ?>
...
<?php if ($model->result == Solution::OJ_CE || !Yii::$app->setting->get('isContestMode') || (!Yii::$app->user->isGuest && Yii::$app->user->identity->isAdmin())) : ?>
